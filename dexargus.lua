...
local cloneref = if not waxcloneref then function(...) return ... end else setmetatable({}, {__mode = 'v', __call = function(self, obj)
    ...
    -- Keep LuaSourceContainer as original userdata. cloneref remaps Instance cache and
    -- breaks getscriptbytecode/decompile (native AV). Plain dex.lua never clonerefs scripts.
    if typeof(obj) == "Instance" then
        local ok, is_src = pcall(function()
            return obj:IsA("LuaSourceContainer")
        end)
        if ok and is_src then
            return obj
        end
    end
		...
		local function main()
			...
			Properties.ShowExplorerProps = function()
				...
				for i = 1,#sList do
					...
					pcall(function() is_script = obj:IsA("LuaSourceContainer") end)

					local hiddenProps = {}
					-- Skip heavy/native prop dumps on scripts — was AV + lag on single-click select.
					if not is_script and env.gethiddenproperties then
						pcall(function()
					...
					local allProps = {}
					if not is_script and showAllProps and env.getproperties then
						pcall(function()
	...
	Properties.DisplayAddTagWindow = function(editTag)
		...
		elseif t == "Instance" then
			...
			Properties.GetPropVal = function(prop,obj)
				...
				-- Source/LinkedSource on scripts: never index (can native-AV).
				if prop.Name == "Source" or prop.Name == "LinkedSource" or prop.Name == "Bytecode" then
					local ok_isa, is_src = pcall(function() return obj:IsA("LuaSourceContainer") end)
					if ok_isa and is_src then
						return "<Protected>"
					end
				end
				...
				elseif prop.IsTag then
					...
					if s then
						...
						-- Do not call gethiddenproperty on scripts (flag flip + index AV).
						local ok_isa, is_src = pcall(function() return obj:IsA("LuaSourceContainer") end)
						if ok_isa and is_src then
							return nil
						end
...
local function MainFunc()
	...
	if env.getfunctionhash then 
		...
		if S and Hash then return Hash end
	end
...
local function ViewTableDetail(Tab, Name)
	...
	DetailCodeFrame:SetText("-- Serializing table, please wait...")
	DetailWindow:Show()

	task.spawn(function()
		local Ok, Content = pcall(function()
			return SerializeData.Convert(Tab, true)
		end)
		DetailCodeFrame:SetText(Ok and Content or ("Error reading table: " .. tostring(Content)))
	end)
end
...
function SerializeData.Convert(value, use_indent)
	...
	local function ShowModifyValueDialog(Match, Callback)
		local PromptWin = Lib.Window.new()
		PromptWin.Alignable = false
		PromptWin.Resizable = false
...
if not table.find(TypeOptions, CurrentType) then
    CurrentType = "string"
end
TypeDropdown:SetSelected(CurrentType)
		...
		Btn.Size = UDim2.new(1, -10, 0, 20)
		Btn.OnClick:Connect(function()
			local SelectedType = TypeDropdown.Selected or "string"
			local RawInput = Box:GetText()
			Callback(SelectedType, RawInput)
			PromptWin:Close()
		end)
		PromptWin:Add(Btn)
		PromptWin:Show()
	...
	local function ParseToType(TypeStr, ValStr)
		if TypeStr == "string" then
			return ValStr
		elseif TypeStr == "number" then
			return tonumber(ValStr) or 0
		elseif TypeStr == "boolean" then
			return ValStr:lower() == "true"
		end
		
		if env.loadstring then
			local SourceCode
			if TypeStr == "Color3" and not ValStr:find("Color3") then
				SourceCode = "return Color3.fromRGB(" .. ValStr .. ")"
			...
			elseif TypeStr == "DateTime" and not ValStr:find("DateTime") then
				SourceCode = "return DateTime.fromUnixTimestampMillis(" .. ValStr .. ")"
			else
				SourceCode = "return " .. ValStr
			end
	...
	if not IsCClosure then
		...
		InfoStr = InfoStr .. "Constants (" .. ((S_Const and type(Constants) == "table") and #Constants or 0) .. "):\n"
		...
		InfoStr = InfoStr .. "\nUpvalues (" .. ((S_Upv and type(Upvalues) == "table") and #Upvalues or 0) .. "):\n"
		...
		InfoStr = InfoStr .. "\nProtos (" .. ((S_Protos and type(Protos) == "table") and #Protos or 0) .. "):\n"
...
local func = findfunc("]] .. tostring(Target) .. [[")]]
					...
					FuncContextMenu:Add({Name = "View Script", OnClick = function()
						...
						if Scr then 
							...
							DetailCodeFrame:SetText("-- couldn't find any")
							DetailWindow:Show()
						end
					end})
...
local tbl = findtable()]]
		...
		ValueContextMenu:Add({Name = "Modify Value", OnClick = function() 
			...
			ShowModifyValueDialog(Match, function(SelectedType, RawInput)
				local ParsedVal = ParseToType(SelectedType, RawInput)
...
local DebugExplorer = {}
	...
	local function ApplyInstantHook(ActionName, Code, TargetFunc)
		if env.loadstring then
			...
			if TargetFunc then
				DebugExplorer.HookedFunctions[GetHash(TargetFunc)] = Code
			else
				local HookData = {
	...
	local function GetFuncDisplayName(Func)
		local s, n = pcall(debug.info, Func, "n")
		local name = (s and type(n) == "string" and n ~= "") and n or "Anonymous"
	...
	local function ShowModifyValueDialog(Match, Callback)
		...
		PromptWin.Alignable = false
		PromptWin.Resizable = false
		PromptWin:SetTitle("Modify Upvalue")
		PromptWin:SetSize(320, 160)
		...
		TypeDropdown:SetOptions({
			"string", "number", "boolean", "Axes", "BrickColor", "CFrame", "Color3", 
			"ColorSequence", "ColorSequenceKeypoint", "DateTime", "DockWidgetPluginGuiInfo", 
			"Enum", "EnumItem", "Faces", "Instance", "NumberRange", "NumberSequence", 
			"OverlapParams", "PathWaypoint", "PhysicalProperties", "Random", "Ray", "RaycastParams", 
    ...
    return o(...)
			...
			elseif Type == "Event" then
				...
				EventContextMenu:Add({Name = "Fire Signal", OnClick = function()
					...
					ShowInputPrompt("Arguments (e.g. 1, 'String')", "", function(StrArgs)
							...
							for cIdx, cVal in next, Constants do
								...
								local CNode = CreateTreeItem(ConstNode, BaseIndent + 2, cPrefix, string.format("[%d] = %s <font color='rgb(130,130,130)'>(%s)</font>", cIdx, cDisp:gsub("<","&lt;"):gsub(">","&gt;"), cType), Color3.fromRGB(180, 180, 180), isFunc, nil, function()
									...
									ConnConstContextMenu:Add({Name = "Modify Value", OnClick = function() 
										ShowModifyValueDialog({Index = cIdx, Value = cVal, ValueType = cType}, function(SelectedType, RawInput)
											local ParsedVal = ParseToType(SelectedType, RawInput)
									...
									if isFunc then
										ConnConstContextMenu:Add({Name = "Detailed View", OnClick = function()
											if ViewFunctionDetail then ViewFunctionDetail(cVal, "Constant ["..cIdx.."]", GetHash(cVal)) end
										end})
									end
							...
							for uIdx, uVal in next, Upvs do
								...
								local UNode = CreateTreeItem(UpvNode, BaseIndent + 2, uPrefix, string.format("[%d] = %s <font color='rgb(130,130,130)'>(%s)</font>", uIdx, uDisp:gsub("<","&lt;"):gsub(">","&gt;"), uType), Color3.fromRGB(180, 180, 180), isFunc, nil, function()
									...
									ConnUpvalContextMenu:Add({Name = "Modify Value", OnClick = function() 
										ShowModifyValueDialog({Index = uIdx, Value = uVal, ValueType = uType}, function(SelectedType, RawInput)
											local ParsedVal = ParseToType(SelectedType, RawInput)
									...
									if isFunc then
										ConnUpvalContextMenu:Add({Name = "Detailed View", OnClick = function()
											if ViewFunctionDetail then ViewFunctionDetail(uVal, "Upvalue ["..uIdx.."]", GetHash(uVal)) end
										end})
									end
				...
				if ConnCount > 0 then
					for Idx, C in next, Conns do
						...
						if type(C_Func) == "function" then
							...
							local FuncStr = "Function: " .. GetFuncDisplayName(C_Func) .. BlockTag .. HookTag
							...
							local FnNode = CreateTreeItem(ConnNode, 3, "└──", FuncStr, Color3.fromRGB(150, 200, 255), HasProps, nil, function()
								...
								ConnFuncContextMenu:Add({Name = "Detailed View", OnClick = function() 
									if ViewFunctionDetail then ViewFunctionDetail(C_Func, "ConnectedFunction", GetHash(C_Func)) end
								end})
...
local o; o = hookfunction(target, function(...)
    ...
    return o(...)
							...
							CreateTreeItem(ConnNode, 3, "└──", "Function: C Closure (Core Engine)", Color3.fromRGB(255, 150, 150), false, nil, nil, ConnID .. "_CClose")
						end
					end
				end
			end
	...
	DebugExplorer.Init = function()
		...
		DepthToggleBtn.MouseButton1Click:Connect(function()
			ShowInputPrompt("Set Tree Depth Limit", DebugExplorer.MaxDepth, function(StrVal)
				local Num = tonumber(StrVal)
				if Num then 
...
local o; o = hookfunction(getrawmetatable(game).__index, newcclosure(function(...)
    ...
    return o(...)
		...
		ConnFuncContextMenu:Add({Name = "Detailed View", OnClick = function() 
			...
			if Func and ViewFunctionDetail then 
				ViewFunctionDetail(Func, "ConnectedFunction", GetHash(Func)) 
			end
		end})
...
local o; o = hookfunction(target, function(...)
    ...
    return o(...)
		...
		ConnUpvalContextMenu:Add({Name = "Modify Value", OnClick = function() 
			...
			ShowModifyValueDialog(Upv, function(SelectedType, RawInput)
				local ParsedVal = ParseToType(SelectedType, RawInput)
		...
		ConnConstContextMenu:Add({Name = "Modify Value", OnClick = function() 
			...
			ShowModifyValueDialog(Cst, function(SelectedType, RawInput)
				local ParsedVal = ParseToType(SelectedType, RawInput)
				...
				if S then
					...
					if HookWindow.GuiElems.Title.Text == "Hooking Connected Function" and DebugExplorer.SelectedConnectionFunc then
						...
						table.insert(DebugExplorer.ActiveHooks, {
							...
							Type = "Conn Function",
							Name = "Func: " .. GetFuncDisplayName(DebugExplorer.SelectedConnectionFunc),
							Action = "Hooked",
							ScriptSource = Source,
...
DefaultSettings = (function()
	...
	return {
		...
		SaveInstance = {
			Method = "UniversalSynSaveInstance"
		},
...
if oth and oth.hook then
    ...
    env.hookfunction = function(f1, f2)
		...
		-- Freeze executor APIs before optional fallback loaders can touch them.
		local executor_decompile = decompile
		local executor_getscriptbytecode = getscriptbytecode

		local function safe_getscriptbytecode(scr)
			if typeof(scr) ~= "Instance" then return nil end
			local ok_isa, is_src = pcall(function() return scr:IsA("LuaSourceContainer") end)
			if not ok_isa or not is_src then return nil end
			if type(executor_getscriptbytecode) ~= "function" then return nil end
			local ok, bytecode = pcall(executor_getscriptbytecode, scr)
			if not ok or type(bytecode) ~= "string" or #bytecode == 0 then return nil end
			return bytecode
		end

		env.getscriptbytecode = safe_getscriptbytecode

		-- DECOMPILERS (lazy — do not load AdvancedDecompiler at boot; it is heavy and unused unless selected)
		local AdvancedDecompilerCache
		local function get_advanced_decompiler()
			if AdvancedDecompilerCache ~= nil then return AdvancedDecompilerCache end
			...
			AdvancedDecompilerCache = (ok and res) or false
		...
		local function KonstantDec(scriptPath)
			local API = "http://api.plusgiant5.com"
			local bytecode = safe_getscriptbytecode(scriptPath)
			if not bytecode then return "-- Failed to get script bytecode\n" end
			local time_elapsed = os.clock() - konstant_last_call
			if time_elapsed <= .5 then task.wait(.5 - time_elapsed) end
			local okReq, httpResult = pcall(env.request, {
				Url = API .. "/konstant/decompile", Body = bytecode, Method = "POST", Headers = {["Content-Type"] = "text/plain"}
			})
			konstant_last_call = os.clock()
			if not okReq or type(httpResult) ~= "table" then return "-- Error occurred while requesting Konstant API\n" end
			if httpResult.StatusCode ~= 200 then return "-- Error occurred while requesting Konstant API\n" end
			return httpResult.Body or "-- empty Konstant body\n"
		end

		local function ADDec(...)
			local dec = get_advanced_decompiler()
			if type(dec) ~= "function" then return "Failed to load Advanced Decompiler" end
			local ok, out = pcall(dec, ...)
			if not ok then return "-- AdvancedDecompiler error: " .. tostring(out) end
			return out
		end

		local lua_expert_last = 0
		local function LuaExpertDec(scr)
			local bytecode = safe_getscriptbytecode(scr)
			if not bytecode then return "-- failed to read script bytecode" end

			local elapsed = os.clock() - lua_expert_last
			if elapsed < 0.12 then task.wait(0.12 - elapsed) end

			local okReq, res = pcall(env.request, {
			...
			if not okReq or type(res) ~= "table" or res.StatusCode ~= 200 then
				return "-- api request error\n--[[\n" .. tostring(res and res.Body or res or "no response") .. "\n--]]"
			end
			return res.Body
		end
		...
		local function ShinyDec(script_instance)
			...
			local bytecode = safe_getscriptbytecode(script_instance)
			if not bytecode then return "-- failed to read script bytecode" end
			local encoded = base64Encode(bytecode)
			local okReq, res = pcall(env.request, {
				Url = "http://127.0.0.1:"..tostring(port).."/luau/decompile", Method = "POST", Body = encoded
			})
			if not okReq or type(res) ~= "table" then return "-- Shiny request failed" end
			return res.Body
		end
		...
		local function SabreDec(scr, mode)
			...
			if not okReq or type(res) ~= "table" or not res.Body then
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Desc",Parent={3},Position=UDim2.new(0,-230,0,60),Size=UDim2.new(0,180,0,25),Text="ArgusX Edition",TextColor3=Color3.new(1,1,1),TextSize=18,TextTransparency=1,}},
...
cloneref(game:GetService("WebViewService")):CloseWindow()
	...
	Main.Init = function()
		...
		Main.LoadSettings()
		-- Saved settings often re-enable ShowAll/ShowHidden — that dumps every prop via
		-- getproperties/__index and AVs on LocalScript/ModuleScript single-click select.
		if Settings.Properties then
			Settings.Properties.ShowAllProperties = false
			Settings.Properties.ShowHidden = false
		end
